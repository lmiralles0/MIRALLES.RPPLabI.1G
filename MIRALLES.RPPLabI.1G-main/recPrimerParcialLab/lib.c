...
static void inicializarContador(int* contador, int tam);
...
int clienteConMasAlquileres(eCliente* arrayCliente, int longitudC, eAlquiler* arrayAlquiler, int longitudA)
{
    ...
    int indice;
    int maximo = 0;
    int contador[longitudC];
    inicializarContador(contador, longitudC);
    if(arrayAlquiler != NULL && longitudA > 0)
    {
        for(int i = 0; i < longitudC; i++)
        {
            ...
            {
                if(arrayAlquiler[j].clienteAlquiler.id == arrayCliente[i].id && arrayCliente[i].isEmpty == 0 && arrayAlquiler[j].isEmpty == 0)
                {
                    contador[i]++;
                }
            }
        }
        for(int x = 0; x < longitudC; x++)
        {
        ...
        for(int y = 0; y < longitudC; y++)
        {
            if(contador[indice] == contador[y])
            {
                mostrarCliente(arrayCliente[y]);
            }
        }
    }
    return retorno;
    ...
    if(arrayEquipos != NULL && arrayAlquiler != NULL && longitudA > 0 && longitudE> 0)
    {
        for(int i = 0; i < longitudA; i++)
        {
            for(int j = 0; j < longitudE; j++)
            {
                ...
                if(arrayAlquiler[i].equipo == arrayEquipos[j].id && arrayAlquiler[i].isEmpty == 0)
                {
    ...
    if(arrayAlquiler != NULL && arrayCliente != NULL && longitudC > 0 && longitudA > 0)
    {
        ...
        for(int i = 0; i < longitudC; i++)
        {
            for(int j = 0; j < longitudA; j++)
            {
                ...
                if(arrayCliente[i].isEmpty == 0 && arrayAlquiler[j].isEmpty == 0 && arrayCliente[i].id == arrayAlquiler[j].clienteAlquiler.id)
                {
                ...
                if(arrayCliente[i].isEmpty == 0 && arrayAlquiler[j].isEmpty == 1 && arrayCliente[i].id == arrayAlquiler[j].clienteAlquiler.id)
                {
                    cargarDescripcionEquipos(buffer, arrayEquipo, longitudE, arrayAlquiler[j].equipo);
